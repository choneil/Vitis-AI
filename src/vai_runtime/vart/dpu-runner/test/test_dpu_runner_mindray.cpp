...
int main(int argc, char* argv[]) {
  ...
  pid_t pid_rm = fork();
  if (pid_rm == 0) {
    std::cout << "[Child rm.xclbin] PID " << getpid() << std::endl;
    run_inference(xmodel, kernel, "firmware: /usr/lib/rm.xclbin");
  ...
  pid_t pid_rp = fork();
  if (pid_rp == 0) {
    std::cout << "[Child rp.xclbin] PID " << getpid() << std::endl;
    // system("env XLNX_VART_FIRMWARE=/usr/lib/rp.xclbin show_dpu");
    // std::cout<<"version info"<<std::endl;
    // system("devmem 0xa4200000");
    run_inference(xmodel, kernel, "firmware: /usr/lib/rp.xclbin");
     std::cout<<"version info"<<std::endl;
    system("devmem 0xa4200000");
    exit(0);
  }
  ...
  waitpid(pid_rp, &status, 0);
  std::cout << "Both children completed." << std::endl;
  return 0;
