...
def main(unused_argv):
  ...
  # skip_op_list = []
  new_var_list = []
  model_dir = './finetune_dir_pruned0.85_bak'
  ...
  with tf.Session() as sess:
    for op_name, tensor in original_tensor_dict.items():
      ...
      if op_name in merge_conv_op_list:
        ...
        if "Momentum" in op_name:
          var_name = op_name.split("/Momentum")[0]
          var_with_momentum = original_tensor_dict[var_name]
          new_tensor = tf.Variable(initial_value=np.zeros_like(var_with_momentum),
                  name=op_name, dtype=var_with_momentum.dtype,
                  shape=var_with_momentum.shape)
        else:
