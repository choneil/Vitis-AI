...
YOLOv5Imp::YOLOv5Imp(const std::string& model_name, bool need_preprocess)
    : YOLOv5(model_name, need_preprocess),
      tf_flag_(configurable_dpu_task_->getConfig().is_tf()) {}

YOLOv5Imp::~YOLOv5Imp() {}

YOLOv5Result YOLOv5Imp::run(const cv::Mat& input_images) {
  cv::Mat image;
  int sWidth = getInputWidth();
  int sHeight = getInputHeight();
  auto mAP = configurable_dpu_task_->getConfig().yolo_v5_param().test_map();
  auto type = configurable_dpu_task_->getConfig().yolo_v5_param().type();
...
//# DPUV1 needs float input data
      ...
      LOG_IF(INFO, false) << "scale " << scale << " "      //
                          ...
                          << "sHeight " << sHeight << " "  //
                          << std::endl;
      yolov5::convertInputImage(input_images, sWidth, sHeight, channel, scale,
                                data);
    } else {
      if (type == 1) {  // 1: yolov5-large/yolov5-nano/yolov5s6
        image = yolov5::letterbox(input_images, sWidth, sHeight).clone();
      } else {
        image = yolov5::letterbox_tf(input_images, sWidth, sHeight).clone();
      }
    ...
    //  convert_RGB(image);
    __TIC__(YOLOV4_SET_IMG)
    configurable_dpu_task_->setInputImageRGB(image);
    __TOC__(YOLOV4_SET_IMG)
  }
  __TIC__(YOLOV4_DPU)
  configurable_dpu_task_->run(0);
  __TOC__(YOLOV4_DPU)

  __TIC__(YOLOV4_POST_ARM)

  auto ret = vitis::ai::yolov5_post_process(
  ...
  __TOC__(YOLOV4_POST_ARM)
  return ret;
}  // namespace ai

vector<YOLOv5Result> YOLOv5Imp::run(const vector<cv::Mat>& input_images) {
  cv::Mat image;
  int sWidth = getInputWidth();
  int sHeight = getInputHeight();
  auto mAP = configurable_dpu_task_->getConfig().yolo_v5_param().test_map();
...
//# DPUV1 needs float input data
        ...
        LOG_IF(INFO, false) << "scale " << scale << " "      //
                            ...
                            << "sHeight " << sHeight << " "  //
                            << std::endl;
        yolov5::convertInputImage(input_images[i], sWidth, sHeight, channel,
                                  scale, data);
      }
    } else {
      ...
      for (auto input_image : input_images) {
        if (type == 1) {  // 1: yolov5-large/yolov5-nano/yolov5s6
          images.push_back(
              yolov5::letterbox(input_image, sWidth, sHeight).clone());
        } else {
          images.push_back(
              yolov5::letterbox_tf(input_image, sWidth, sHeight).clone());
        }
      }
...
vector<YOLOv5Result> YOLOv5Imp::run(const vector<vart::xrt_bo_t>& input_bos) {
  __TIC__(YOLOV4_DPU)
