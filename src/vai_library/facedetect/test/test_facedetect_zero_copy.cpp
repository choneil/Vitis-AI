...
int main(int argc, char* argv[]) {
  ...
  auto bos = std::vector<std::unique_ptr<xrt::bo>>();
  for (auto batch_idx = 0u; batch_idx < batch; ++batch_idx) {
    bos.emplace_back(std::make_unique<xrt::bo>(*dev, input_tensor_size, 0));
    bo_for_inputs.push_back(vart::xrt_bo_t{dev.get(), bos[batch_idx].get()});
  }
  ...
  for (auto batch_idx = 0u; batch_idx < bo_for_inputs.size(); batch_idx++) {
    auto xrt_bo =
        reinterpret_cast<xrt::bo*>(bo_for_inputs[batch_idx].xrt_bo_handle);
    auto data = xrt_bo->map<int*>();
