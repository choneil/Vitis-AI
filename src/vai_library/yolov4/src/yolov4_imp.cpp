...
YOLOv4Imp::YOLOv4Imp(const std::string& model_name, bool need_preprocess)
    : YOLOv4(model_name, need_preprocess),
      tf_flag_(configurable_dpu_task_->getConfig().is_tf()) {}

YOLOv4Imp::~YOLOv4Imp() {}

YOLOv4Result YOLOv4Imp::run(const cv::Mat& input_images) {
  cv::Mat image;
  int sWidth = getInputWidth();
  int sHeight = getInputHeight();
  auto mAP = configurable_dpu_task_->getConfig().yolo_v4_param().test_map();
  auto type = configurable_dpu_task_->getConfig().yolo_v4_param().type();
...
//# DPUV1 needs float input data
      ...
      LOG_IF(INFO, false) << "scale " << scale << " "      //
                          ...
                          << "sHeight " << sHeight << " "  //
                          << std::endl;
      yolov4::convertInputImage(input_images, sWidth, sHeight, channel, scale,
                                data);
    } else {
      if (type == 1) {  // 1: yolov4-csp;
        image = yolov4::letterbox(input_images, sWidth, sHeight).clone();
      } else {
        image = yolov4::letterbox_tf(input_images, sWidth, sHeight).clone();
      }
  ...
  auto ret = vitis::ai::yolov4_post_process(
...
vector<YOLOv4Result> YOLOv4Imp::run(const vector<cv::Mat>& input_images) {
  cv::Mat image;
  int sWidth = getInputWidth();
  int sHeight = getInputHeight();
  auto mAP = configurable_dpu_task_->getConfig().yolo_v4_param().test_map();
...
//# DPUV1 needs float input data
        ...
        LOG_IF(INFO, false) << "scale " << scale << " "      //
                            ...
                            << "sHeight " << sHeight << " "  //
                            << std::endl;
        yolov4::convertInputImage(input_images[i], sWidth, sHeight, channel,
                                  scale, data);
      }
    } else {
      ...
      for (auto input_image : input_images) {
        if (type == 1) {  // 1: yolov4-csp;
          images.push_back(
              yolov4::letterbox(input_image, sWidth, sHeight).clone());
        } else {
          images.push_back(
              yolov4::letterbox_tf(input_image, sWidth, sHeight).clone());
        }
      }
...
vector<YOLOv4Result> YOLOv4Imp::run(const vector<vart::xrt_bo_t>& input_bos) {
  __TIC__(YOLOV4_DPU)
