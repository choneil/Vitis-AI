...
static vitis::ai::library::InputTensor convert_tensor_buffer_to_input_tensor(
  ...
  for (auto batch_idx = 0; batch_idx < dims[0]; ++batch_idx) {
    ...
    if (tb_ext) {
      ...
      // should use xrt::device* and xrt::bo* directly
      ret.xcl_bo[batch_idx].xcl_handle = const_cast<void*>(bo.xcl_handle);
      // due to const void* to unsinged int compilation error, set to 0
      ret.xcl_bo[batch_idx].bo_handle = 0;
