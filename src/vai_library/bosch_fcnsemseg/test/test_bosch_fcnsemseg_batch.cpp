...
int main(int argc, char *argv[]) {
  ...
  auto seg =
      vitis::ai::Bosch_Fcnsemseg::create(argv[1]);
  ...
  auto results = seg->run(batch_images);
  for (auto batch_idx = 0u; batch_idx < results.size(); batch_idx++) {
    ...
    filenamepart1 += "_batch_result.png";
    cv::imwrite(filenamepart1, results[batch_idx].segmentation);
  }
  return 0;
}
