...
def runThread(id, start, encoder_dpu_runner, decoder_dpu_runner, img):
  ...
  while count < n_of_images:
    ...
    for j in range(runSize):
      ...
      img[(count + j) % n_of_images].resize((28, 28, 1), refcheck=False)
    ...
    for j in range(runSize):
     # sampleRun = decoder_dict['quant_sampling_reshaped_inserted_fix_5']
     # sampleRun[j, ...] = sample_z[j].reshape(tuple(decoder_dict['quant_sampling_reshaped_inserted_fix_5'].shape[1:]))
     sampleRun = decoder_dict['quant_sampling']
     sampleRun[j, ...] = sample_z[j].reshape(tuple(decoder_dict['quant_sampling'].shape[1:]))
