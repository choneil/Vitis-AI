...
cv::Mat draw_bev(const std::vector<vitis::ai::CenterPointResult>& cr,
  ...
  */
  auto img = cv::Mat(int(BEV_SHOW_SIZE * scale), int(BEV_SHOW_SIZE * scale),
  ...
  for (int i = 0; i < (int)corner_full.size(); i++) {
    for (int j = 0; j < 4; j++) {
      pt1.x = ((corner_full[i][line_idx[j][0]][0] - bx[0]) / dx[0]) * scale;
      pt1.y = myfix_y(((corner_full[i][line_idx[j][0]][1] - bx[1]) / dx[1])) *
              scale;
      pt2.x = ((corner_full[i][line_idx[j][1]][0] - bx[0]) / dx[0]) * scale;
  ...
  V2F pts{{-4.084 / 2. + 0.5, 1.85 / 2.},
          ...
          {-4.084 / 2. + 0.5, -1.85 / 2.}};
  for (int j = 0; j < 4; j++) {
    ptsx[j].y = ((pts[j][0] - bx[0]) / dx[0]) * scale;
    ptsx[j].x = ((pts[j][1] - bx[1]) / dx[1]) * scale;
  }
