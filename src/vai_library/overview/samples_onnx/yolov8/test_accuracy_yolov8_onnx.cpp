...
struct Yolov8Acc : public AccThread {
  Yolov8Acc(std::string output_file)
      : AccThread(), of(output_file, std::ofstream::out) {
    dpu_result.frame_id = -1;
  }

  virtual ~Yolov8Acc() { of.close(); }

  static std::shared_ptr<Yolov8Acc> instance(std::string output_file) {
    static std::weak_ptr<Yolov8Acc> the_instance;
    std::shared_ptr<Yolov8Acc> ret;
    if (the_instance.expired()) {
      ret = std::make_shared<Yolov8Acc>(output_file);
      the_instance = ret;
    }
  ...
  void process_result(DpuResultInfo dpu_result) {
    ...
    auto result = (Yolov8OnnxResult*)dpu_result.result_ptr.get();
...
int main(int argc, char* argv[]) {
  ...
  return vitis::ai::main_for_accuracy_demo(
      argc, argv, [&] { return Yolov8Onnx::create(model_name, 0.001); },
      vitis::ai::Yolov8Acc::instance(argv[3]), 2);
}
