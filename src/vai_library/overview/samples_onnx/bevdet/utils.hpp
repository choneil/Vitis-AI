...
std::pair<std::vector<int8_t>, std::vector<uint32_t>> topK(
    const float scores[], int scores_num, int begin, uint32_t topk, bool sorted,
    float score_threshold) {
  using elem_t = std::pair<int8_t, uint32_t>;
  std::vector<elem_t> queue;
  for (auto j = 0; j < scores_num; j++) {
    if (scores[j + begin] > score_threshold) {
      queue.push_back({scores[j + begin], j});
    }
  }
...
std::vector<uint32_t> remainder_n(const std::vector<uint32_t>& src,
                                  uint32_t dividend) {
...
std::vector<uint32_t> division_n(const std::vector<uint32_t>& src,
                                 uint32_t dividend) {
  std::vector<uint32_t> dst(src.size());
  for (size_t i = 0; i < src.size(); i++) {
    dst[i] = src[i] / dividend;
...
std::vector<float> gather_featHW(const float input[],
                                 const std::vector<uint32_t>& index,
                                 size_t cat) {
  std::vector<float> dst(index.size() * cat);
  auto dststep = index.size();
  for (auto i = 0u; i < index.size(); i++) {
    for (size_t j = 0; j < cat; j++) {
      dst[j * dststep + i] = input[index[i] + j * HW];
    }
  }
  return dst;
}
...
void xywhr2xyxyr(const float box_xywhr[9], float* dst) {
...
int check_in_box2d(const float for_box2d[], const float box[], const Point& p) {
...
float box_overlap(const std::vector<bool>& is_first, float boxes[][5], size_t a,
                  size_t b, Point box_corners[][5], float for_box2d[][4]) {
...
float iou_bev(std::vector<bool>& is_first, float box_areas[],
              Point box_corners[][5], float for_box2d[][4], float boxes[][5],
              size_t a, size_t b) {
  // params: box_a (5) [x1, y1, x2, y2, angle]
  // params: box_b (5) [x1, y1, x2, y2, angle]
  if (is_first[a])
...
//                            vart::TensorBuffer* input, std::vector<float>
//                            mean, std::vector<float> scale) {
...
inline int decode(Ort::Value* regtb, Ort::Value* heitb, Ort::Value* dimtb,
                  Ort::Value* rottb, Ort::Value* veltb, Ort::Value* heatmaptb,
                  float score_threshold, uint32_t& first_label,
                  CenterPointResult* res) {
  auto conf_desigmoid = -logf(1.0f / score_threshold - 1.0f);
  // auto output_tensor_ptr = output_tensor.;
  auto heatmap = heatmaptb->GetTensorData<float>();
  // process heat
  auto output_shape = heatmaptb->GetTensorTypeAndShapeInfo().GetShape();
  LOG(INFO) << "[" << output_shape[0] << "," << output_shape[1] << ","
            << output_shape[2] << "," << output_shape[3] << "]";
  auto cat = output_shape[1];
  ...
  if (cat == 1) {
    std::tie(top_scores, top_inds) =
        topK(heatmap, HW, 0, K, true, conf_desigmoid);
    ...
    std::tie(topk_scores, topk_inds) =
        topK(heatmap, HW, 0, K, false, conf_desigmoid);
    auto cls_flag = topk_inds.size();
    auto temp = topK(heatmap, HW, HW, K, false, conf_desigmoid);
  ...
  if (top_scores.size() > 0) {
    auto reg = regtb->GetTensorData<float>();
    auto hei = heitb->GetTensorData<float>();
    auto rot = rottb->GetTensorData<float>();
    auto dim = dimtb->GetTensorData<float>();
    auto vel = veltb->GetTensorData<float>();
    // process reg
    auto top_reg = gather_featHW(reg, top_inds, 2);
    auto top_hei = gather_featHW(hei, top_inds, 1);
    auto top_dim = gather_featHW(dim, top_inds, 3);
    auto top_rot = gather_featHW(rot, top_inds, 2);
    auto top_vel = gather_featHW(vel, top_inds, 2);

    // std::vector<CenterPointResult> res;
    auto step = top_scores.size();
    for (auto i = 0u; i < step; i++) {
      if (top_hei[i] >= post_center_range[2] &&
          top_hei[i] <= post_center_range[5]) {
        float xs = (top_xs[i] + top_reg[i]) * out_size_factor * voxel_size[0] +
                   pc_range[0];
        float ys =
            (top_ys[i] + top_reg[i + step]) * out_size_factor * voxel_size[1] +
            pc_range[1];
        if (xs >= post_center_range[0] && ys >= post_center_range[1] &&
          ...
          res[res_num].bbox[3] = exp(top_dim[i]);
          res[res_num].bbox[4] = exp(top_dim[i + step]);
          res[res_num].bbox[5] = exp(top_dim[i + 2 * step]);
          res[res_num].bbox[2] = top_hei[i] - res[res_num].bbox[5] * 0.5f;
          res[res_num].bbox[6] = atan2(top_rot[i], top_rot[i + step]);
          res[res_num].bbox[7] = top_vel[i];
          res[res_num].bbox[8] = top_vel[i + step];
          res[res_num].score = sigmoid_n(top_scores[i]);
...
std::vector<CenterPointResult> post_process(
  ...
  CenterPointResult decode_res[K];
  float bboxes[K][5];
  float box_areas[K];
  float for_box2d[K][4];
  Point box_corners[K][5];
  uint32_t first_label = 0;
  for (size_t i = 0; i < output_tensors.size(); i += 6) {
    //__TIC__(post_process_decode);

    //"reg", "height", "dim", "rot", "vel",    "heatmap";
    auto decode_res_num = decode(&output_tensors[i], &output_tensors[i + 1],
                                 &output_tensors[i + 2], &output_tensors[i + 3],
                                 &output_tensors[i + 4], &output_tensors[i + 5],
