...
inline BenchMarkResult thread_main_for_performance(const ImageList* image_list,
  ...
  while (!g_stop) {
    ...
    for (size_t i = 0; i < batch; i++) {
      auto img_iso = (*image_list)[ret];
      ret++;
      imgs.push_back(img_iso.first);
      isos.push_back(img_iso.second);
    }
    model->run(imgs, isos);
...
inline int main_for_performance(int argc, char* argv[], T factory_method) {
  ...
  auto image_list = std::unique_ptr<ImageList>(new ImageList(g_list_name));
  if (image_list->empty()) {
    LOG(FATAL) << "list of images are empty ";
  }
