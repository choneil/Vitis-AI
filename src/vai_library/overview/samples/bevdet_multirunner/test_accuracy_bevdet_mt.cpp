...
void accuracy_thread(vitis::ai::BEVdet* det, int i, int t_n,
  ...
  for (int ii = start; ii < start + end; ii++) {
    if (i == 0) {
      auto pr = float(int(float(ii - start) / float(end) * 1000.0)) / 10.0;
      auto etime = (double)(std::clock() - start_time) / 1000000.0;
      std::cout << "Progress rate " << pr << "%, elapsed: " << etime
                << "s, ETA: " << etime / pr * 100.0 - etime << "s \r"
                << std::flush;
    }
    images.clear();
    bins.clear();
...
int main(int argc, char* argv[]) {
  if (argc < 5) {
    std::cerr << "usage : " << argv[0]
              << "<model_name0>  <list_file> <dataset_dir> <output_file> "
                 "[thread_num]\n"
  ...
  for (int i = 0; i < t_n; i++) {
    vdet.emplace_back(vitis::ai::BEVdet::create(argv[1]));
