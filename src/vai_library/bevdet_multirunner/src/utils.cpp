...
std::pair<std::vector<float>, std::vector<uint32_t>> topK(
    const float scores[], int scores_num, int begin, int step, uint32_t topk,
...
std::pair<std::vector<float>, std::vector<uint32_t>> topK(
    const float scores[], int scores_num, uint32_t topk, bool sorted,
    float score_threshold) {
  return topK(scores, scores_num, 0, 1, topk, sorted, score_threshold);
}
static std::pair<std::vector<float>, std::vector<uint32_t>> topK(
    const std::vector<float>& scores, uint32_t topk) {
  using elem_t = std::pair<float, uint32_t>;
...
inline int decode(vart::TensorBuffer* heatmaptb, vart::TensorBuffer* regtb,
                  vart::TensorBuffer* heitb, vart::TensorBuffer* rottb,
                  vart::TensorBuffer* dimtb, vart::TensorBuffer* veltb,
                  float score_threshold, int w, uint32_t& first_label,
                  vitis::ai::CenterPointResult* res) {
  auto conf_desigmoid = -logf(1.0f / score_threshold - 1.0f);
  float* heatmap = (float*)heatmaptb->data({0, 0, 0, 0}).first;
  // process heat
  int cat = heatmaptb->data({0, 0, 0, 0}).second / sizeof(float) / HW;
  ...
  if (top_scores.size() > 0) {
    float* reg = (float*)regtb->data({0, 0, 0, 0}).first;
    float* hei = (float*)heitb->data({0, 0, 0, 0}).first;
    float* rot = (float*)rottb->data({0, 0, 0, 0}).first;
    float* dim = (float*)dimtb->data({0, 0, 0, 0}).first;
    float* vel = (float*)veltb->data({0, 0, 0, 0}).first;
    // process reg
    ...
    for (auto i = 0u; i < top_scores.size(); i++) {
      if (hei[i] >= post_center_range[2] && hei[i] <= post_center_range[5]) {
        float xs =
            (top_xs[i] + top_reg[i * 2]) * out_size_factor * voxel_size[0] +
            pc_range[0];
        float ys =
            (top_ys[i] + top_reg[i * 2 + 1]) * out_size_factor * voxel_size[1] +
            pc_range[1];
        if (xs >= post_center_range[0] && ys >= post_center_range[1] &&
          ...
          res[res_num].bbox[3] = exp(top_dim[3 * i]);
          res[res_num].bbox[4] = exp(top_dim[3 * i + 1]);
          res[res_num].bbox[5] = exp(top_dim[3 * i + 2]);
          res[res_num].bbox[2] = top_hei[i] - res[res_num].bbox[5] * 0.5f;
          res[res_num].bbox[6] = atan2(top_rot[2 * i], top_rot[2 * i + 1]);
          res[res_num].bbox[7] = top_vel[2 * i];
          res[res_num].bbox[8] = top_vel[2 * i + 1];
...
std::vector<vitis::ai::CenterPointResult> post_process(
  ...
  vart::TensorBuffer* out_tensors[6][6];
  int W = 0;
  for (auto&& o : output_tensors) {
    auto name = o->get_tensor()->get_name();
    auto shape = o->get_tensor()->get_shape();
    W = shape[2];
    size_t j = 0;
    for (j = 0u; j < 6u; ++j) {
  ...
  for (auto&& os : out_tensors) {
    ...
    auto decode_res_num = decode(os[0], os[1], os[2], os[3], os[4], os[5],
                                 score_threshold, W, first_label, decode_res);
