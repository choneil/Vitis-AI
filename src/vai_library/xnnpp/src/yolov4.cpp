...
static void detect(vector<vector<float>>& boxes, int8_t* result, int height,
  ...
  auto& yolo_params = config.yolo_v4_param();
...
YOLOv4Result yolov4_post_process(
    const std::vector<vitis::ai::library::InputTensor>& input_tensors,
    const std::vector<vitis::ai::library::OutputTensor>& output_tensors,
    const vitis::ai::proto::DpuModelParam& config, const int w, const int h) {
  return yolov4_post_process(input_tensors, output_tensors, config,
                             vector<int>(1, w), vector<int>(1, h))[0];
}

std::vector<YOLOv4Result> yolov4_post_process(
    const std::vector<vitis::ai::library::InputTensor>& input_tensors,
  ...
  auto num_classes = config.yolo_v4_param().num_classes();
  auto nms_thresh = config.yolo_v4_param().nms_threshold();
  auto conf_thresh = config.yolo_v4_param().conf_threshold();
  auto mAP = config.yolo_v4_param().test_map();
  auto layername =
      std::vector<std::string>(config.yolo_v4_param().layer_name().begin(),
                               config.yolo_v4_param().layer_name().end());
  auto iou_type = config.yolo_v4_param().iou_type();
  ...
  std::vector<YOLOv4Result> res_vec;
  ...
  for (int k = 0; k < batch_size; k++) {
    ...
    vector<YOLOv4Result::BoundingBox> results;
    for (size_t i = 0; i < res.size(); ++i) {
      ...
      if (res[i][res[i][4] + 6] > conf_thresh) {
        YOLOv4Result::BoundingBox yolo_res;
    ...
    res_vec.push_back(YOLOv4Result{sWidth, sHeight, results});
  }
  return res_vec;
}
